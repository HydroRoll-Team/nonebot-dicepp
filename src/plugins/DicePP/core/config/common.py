...
DEFAULT_CONFIG: Dict[str, str] = {
    ...
    CFG_FRIEND_TOKEN: "",
    CFG_GROUP_INVITE: "1",
    CFG_AGREEMENT: BOT_AGREEMENT,
}
DEFAULT_CONFIG_COMMENT: Dict[str, str] = {
    CFG_MASTER: "Master账号, 权限最高, 可以有多个Master",
    CFG_ADMIN: "管理员账号, 拥有次高权限, 可以有多个管理员",
    CFG_FRIEND_TOKEN: "用户申请好友时在验证中输入参数中的文本之一骰娘才会通过, 若字符串为空则通过所有的好友验证",
    CFG_GROUP_INVITE: "好友邀请加群时是否同意, 0为总是拒绝, 1为总是同意",
    CFG_AGREEMENT: "使用协议",
}
...
DEFAULT_CONFIG_COMMENT[
    CFG_WHITE_LIST_GROUP
] = "可填多个单元格, 或用;在同一个单元格分隔不同的群号, 列表中的群不会被自动清除信息或退群"

CFG_WHITE_LIST_USER = "white_list_user"
DEFAULT_CONFIG[CFG_WHITE_LIST_USER] = ""
DEFAULT_CONFIG_COMMENT[
    CFG_WHITE_LIST_USER
] = "可填多个单元格, 或用;在同一个单元格分隔不同的账号, 列表中的账号不会被自动清除信息"


def preprocess_white_list(raw_list: List[str]) -> List[str]:
    result_list: List[str] = []
    for raw_str in raw_list:
        result_list.extend(item.strip() for item in raw_str.split(";") if item.strip())
    return result_list
